...
async function main() {
  const root = await mkdtemp(join(tmpdir(), 'vg-batch-query-recovers-storyboard-frame-'))
  ...
  const projectId = 'batch-query-recovers-storyboard-frame'
  const shotId = 'shot_2'
  ...
  const result = await cloneService.batchQueryStoryboardImages({
    cloneProjectId: projectId,
    shotIds: [shotId],
  })
  const recoveredShot = result.project?.blueprint?.shots?.find((item: any) => item.id === shotId)
  const recoveredFrame = result.project?.storyboardFrames?.find((item: any) => item.shotId === shotId)
  ...
  assert.equal(Number(result.queueSummary?.total || 0), 1)
  assert.equal(Number(result.queueSummary?.done || 0), 1)
  assert.equal(Number(result.queueSummary?.failed || 0), 0)
  console.log('clone batch query storyboard recovers local frame smoke test passed')
}
