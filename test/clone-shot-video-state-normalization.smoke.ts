...
async function main() {
  const root = await mkdtemp(join(tmpdir(), 'vg-shot-video-normalize-'))
  ...
  const videoPath = join(root, 'generated_clip.mp4')
  ...
  assert.equal(synced.project?.shotVideoOutputs?.[0]?.status, 'done')
  assert.equal(synced.project?.shotVideoOutputs?.[0]?.remoteStatus, 'succeeded')
  console.log('clone shot video state normalization smoke test passed')
}
