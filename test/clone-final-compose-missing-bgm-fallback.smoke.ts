...
async function createSourceClip(path: string, color: string, durationSec: number, withAudio = false) {
  await runFfmpeg([
    ...
    ...(
      withAudio
        ? ['-f', 'lavfi', '-i', `sine=frequency=880:duration=${durationSec}`, '-shortest', '-c:a', 'aac']
        : []
    ),
    '-pix_fmt',
    'yuv420p',
...
async function main() {
  const root = await mkdtemp(join(os.tmpdir(), 'clone-final-compose-'))
  ...
  const rendered = await renderViralCloneBatch({
    ...
    outDir,
    count: 1,
    bgmPath: join(root, 'missing-reference-video.mp4'),
    shots: [
  ...
  await assertExists(rendered.reportPath)
  assert.equal(await probeHasAudio(rendered.outputs[0]!), false)
  console.log('clone final compose missing bgm fallback smoke test passed')
}
