...
async function main() {
  ...
  try {
    ...
    assert.match(String(capturedVideoBody?.prompt || ''), /locked-camera near-static hold/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /product fully frozen/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /product pixels as fully frozen/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /only tiny non-product micro-movement/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /do not translate, rotate, swing, bounce, breathe, sway, or drift any visible product region/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /no camera travel/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /no lens breathing feel/i)
    assert.match(String(capturedVideoBody?.prompt || ''), /only tiny non-product micro-movement/i)
    ...
    assert.match(String(capturedVideoBody?.negative_prompt || ''), /product movement/i)
    assert.match(String(capturedVideoBody?.negative_prompt || ''), /product rotation/i)
