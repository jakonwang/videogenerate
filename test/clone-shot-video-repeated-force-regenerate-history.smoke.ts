...
async function main() {
  const root = await mkdtemp(join(tmpdir(), 'vg-shot-video-repeated-force-regenerate-'))
  ...
  const project = await cloneRepo.upsertProject({
    id: 'repeated-force-regenerate-project',
    createdAt: Date.now(),
    updatedAt: Date.now(),
    title: 'repeated-force-regenerate-project',
    archived: false,
    status: 'ready',
    runMode: 'rewrite',
    locale: 'zh-CN',
  ...
  assert.ok(Array.isArray(output?.previousTaskIds))
  assert.ok((output?.previousTaskIds || []).includes('veo_3_1:task_old'))
  assert.equal(new Set(output?.previousTaskIds || []).size, (output?.previousTaskIds || []).length)
  console.log('clone shot video repeated force regenerate history smoke test passed')
}
