...
async function main() {
  const root = await mkdtemp(join(tmpdir(), 'vg-shot-video-batch-retry-old-task-'))
  ...
  const projectId = 'batch-retry-old-task-does-not-restore-project'
    ...
    shotVideoOutputs: [
      {
        ...
        videoUrl: 'https://example.com/old.mp4',
        updatedAt: 2150,
        sourceEvent: 'late_old_batch_worker',
      },
    ],
  } as any)
  ...
  console.log('clone shot video batch retry old task does not restore smoke test passed')
}
