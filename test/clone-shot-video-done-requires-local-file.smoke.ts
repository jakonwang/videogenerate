...
async function main() {
  const root = await mkdtemp(join(tmpdir(), 'vg-shot-video-done-'))
  ...
  const project = await cloneRepo.upsertProject({
    id: 'done-requires-local',
    createdAt: Date.now(),
    updatedAt: Date.now(),
    title: 'done-requires-local',
    archived: false,
    status: 'ready',
    runMode: 'rewrite',
    locale: 'zh-CN',
    ...
    blueprint: {
      shots: [{ id: 'shot_1', index: 0, status: 'generating', generatedTaskId: 'veo_3_1:task_ok' }],
    },
    shotVideoOutputs: [
      {
        shotId: 'shot_1',
  ...
  assert.notEqual(synced.project?.shotVideoOutputs?.[0]?.status, 'done')
  console.log('clone shot video done requires local file smoke test passed')
}
