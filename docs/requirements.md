...
- 目标：
  - 为桌面端图片模型配置增加可选的图片质量参数，允许用户在设置页直接选择，并在实际图片生成调用时同步透传，覆盖分镜图、模特图和商品图相关链路。
- 本轮最小改动：
  - 调整：
    - `src/renderer/src/ui/views/SettingsView.vue`
    - `docs/requirements.md`
- 生效规则：
  - 设置页“能力模型 > 图片能力”区域必须新增：
    - `图片质量`
  - 可选值固定为：
    - `low`
    - `medium`
    - `high`
  - 设置页摘要区必须展示当前图片模型与图片质量，便于用户确认当前实际生效配置。
  - 保存后，图片质量需跟随模型配置一起提交，并复用主进程当前已支持的 `openaiImageQuality` 透传链路。
- 使用说明：
  - 用户可在设置页直接选择图片质量；后续分镜图、模特图和商品图等图片请求会自动带上当前质量参数，无需单独重复设置。
- Windows / Linux 兼容说明：
  - 本轮仅调整桌面端 Vue 设置页展示与文档记录，配置透传继续复用现有 TypeScript 主进程链路，不依赖平台专属能力，Windows 开发与 Linux 部署环境通用。
- 验证：
  - `npm run typecheck`

## 2026-06-06 桌面端重新打包并封版为 v4.2.1

- 目标：
  - 将当前桌面端发布版本从 `v4.2.0` 提升并重新打包为 `v4.2.1`，用于封装本轮 `/clone` 分镜失败诊断、前置拦截与修复引导增强。
- 本轮最小改动：
  - 调整：
    - `package.json`
    - `package-lock.json`
...
- 生效规则：
  - 应用主版本号统一更新为 `4.2.1`。
  - 后续 `electron-builder` 产物文件名将更新为 `VideoGenerate-4.2.1-Setup.exe`。
  - 本轮封版包含以下用户可见修复：
    - 分镜失败时展示真实报错原因与处理建议
    - 缺少模特身份包时提供直达模特页入口
    - 分镜重生成前置条件不满足时直接拦截
    - 分镜批量重生成失败时不再误报“已完成”
- 使用说明：
  - Windows 本地执行 `npm run dist` 后，应在 `release/` 目录看到 `v4.2.1` 对应安装包、`latest.yml` 与 `.blockmap`。
- Windows / Linux 兼容说明：
  - 本轮仅调整 npm 版本元数据、前端交互与文档记录；Windows 开发打包、Linux 部署发布流程保持兼容。
- 验证：
  - `node -p "require('./package.json').version"`
  - `npm run typecheck`
  - `npm run dist`

## 2026-06-06 `/clone` 缺少模特身份包时提供直达模特页的修复入口

- 目标：
  - 当 `/clone` 分镜失败原因是“请先生成并确认新模特身份包”时，减少用户来回查找页面的成本，直接提供可执行的修复入口。
- 本轮最小改动：
  - 调整：
    - `src/renderer/src/ui/views/CloneView.vue`
    - `docs/requirements.md`
- 生效规则：
  - 当当前分镜的真实报错原因匹配“请先生成并确认新模特身份包”时，右侧“真实报错原因”区块必须显示：
    - `去生成身份包`
  - 点击后直接跳转到模特页 `/models`。
- 使用说明：
  - 现在遇到缺少模特身份包的分镜失败时，用户可以直接从当前失败分镜右侧点“去生成身份包”，处理完成后再回到任务继续重生成。
- Windows / Linux 兼容说明：
  - 本轮仅调整 Vue 前端路由跳转与文档记录，不依赖平台专属能力，Windows 开发与 Linux 部署环境通用。
- 验证：
  - `npm run typecheck`

## 2026-06-06 `/clone` 分镜设计面板增加真实报错原因与处理建议展示

- 目标：
  - 让桌面端 `/clone` 在分镜图片生成失败时，用户无需只看笼统的“生成失败”，而是能在当前分镜预览区直接看到真实报错原因与对应处理建议。
- 本轮最小改动：
  - 调整：
    - `src/renderer/src/ui/views/CloneView.vue`
    - `docs/requirements.md`
- 生效规则：
  - 当当前选中的分镜没有生成成功且存在错误信息时，右侧分镜预览卡片必须新增“真实报错原因”区块。
  - 该区块至少展示：
    - 原始失败原因
    - 归类后的错误标题
    - 对应处理建议
  - 当前内置建议覆盖以下常见场景：
    ...
    - `请先完成参考视频分析`
    - `产品标准源生成失败`
    - 模型 / Provider / API Key 配置异常
  - 不允许 redesign；仅在现有分镜预览右侧卡片内增强失败诊断信息。
- 使用说明：
  - 现在在 `/clone` 分镜设计阶段点击失败分镜后，右侧会直接显示真实报错原因和处理建议，便于用户按提示对症修复后再重新生成。
- Windows / Linux 兼容说明：
  - 本轮仅调整 Vue 前端展示逻辑与文档记录，不依赖平台专属能力，Windows 开发与 Linux 部署环境通用。
- 验证：
  - `npm run typecheck`

## 2026-06-06 `/clone` 分镜图片批量重新生成失败时禁止继续提示“已完成”

- 目标：
  - 修复桌面端 `/clone` 中“重新生成失败分镜 / 重新生成选中分镜”在实际仍有分镜图片失败时，前端依旧提示“已完成”的假成功问题。
- 本轮最小改动：
  - 调整：
    - `src/renderer/src/composables/useCloneProjectWorkspace.storyboard.ts`
    - `docs/requirements.md`
- 生效规则：
  - 分镜图片批量重新生成完成后，前端必须检查 `generateAllShotFrames(...)` 返回的 `queueSummary.failed`。
  - 只要 `failed > 0`，本次操作必须按失败处理，不能继续显示：
    - `失败分镜批量重新生成已完成`
    - `选中分镜批量重新生成已完成`
  - 失败时需要优先透出该批次最新的真实分镜错误原因；例如：
    - `请先生成并确认新模特身份包`
  - 只有当本次目标分镜都真正生成成功时，才允许显示成功提示。
- 使用说明：
  - 现在如果分镜图片批量重新生成仍然失败，界面会直接提示真实失败原因，不再出现“列表还是 failed，但顶部提示已完成”的误导状态。
- Windows / Linux 兼容说明：
  - 本轮仅调整 Vue 前端批量重生成结果判定与错误透传，不依赖平台专属能力，Windows 开发与 Linux 部署环境通用。
- 验证：
  - `npm run typecheck`

## 2026-06-06 `/clone` 运行日志弹窗不再把写库调试信息误显示为分镜重生成错误

- 目标：
  - 修复桌面端 `/clone` 里执行“失败分镜批量重新生成”后，运行日志弹窗把内部 `repo-upsert-project` 调试日志直接展示出来，导致用户误以为分镜图片重生成失败的问题。
- 本轮最小改动：
  - 调整：
    - `src/renderer/src/ui/components/RuntimeLogDialog.vue`
    - `docs/requirements.md`
- 生效规则：
  - `/clone` 运行日志弹窗继续显示对用户有操作价值的生成、重试、同步、错误与成功日志。
  - 以下仅供内部排查的调试日志，不再作为运行日志弹窗里的“重要日志”显示：
    - `[clone-debug] repo-upsert-project`
    - `[clone-debug] stage-state`
  - 这样在分镜图片批量重新生成完成后，弹窗不会再把项目写库时附带的旧分镜视频状态误显示成当前分镜图重生成错误。
- 使用说明：
  - 现在在 `/clone` 里重新生成分镜图片后，运行日志会优先展示真正和当前操作直接相关的提交、完成和异常信息，不再把内部写库调试日志混进错误列表。
- Windows / Linux 兼容说明：
  - 本轮仅调整 Vue 端运行日志筛选逻辑与文档记录，不依赖平台专属能力，Windows 开发与 Linux 部署环境通用。
- 验证：
  - `npm run typecheck`
