...
export const pluginDefinitions: PluginDefinition[] = [
  {
    ...
    description:
      "Synchronize GMV MAX campaigns and produce guarded budget and target ROI recommendations through TikTok for Business MCP.",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/tiktok-gmv-max-optimizer",
    usageHint:
      "Connect with TikTok OAuth, review complete-day evidence, and approve low-risk changes from the dedicated workspace.",
    configSchema: [],
  },
  {
    ...
    description:
      "Batch split uploaded videos into commercial material frames, upload them to Qiniu, and feed Hermes product-to-image selection flows.",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/product-image-materials",
    usageHint:
      "Choose a fixed jewelry category, upload source videos, and maintain a reusable image-material library for Hermes Live Photo selection.",
    configSchema: [
      {
        key: "segmentTimeSec",
      ...
      },
      {
        ...
        placeholder: "8",
        description:
          "Maximum number of candidate images returned to Hermes per product selection.",
      },
    ],
  },
  {
    ...
    description:
      "Track SKU baselines, sync shipped quantities from Dianxiaomi, forecast stockout dates, and surface reorder risks.",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/dianxiaomi-inventory",
    usageHint:
      "Sign in to Dianxiaomi, add a SKU baseline, then sync shipped quantities and review the reorder forecast.",
    configSchema: [],
  },
  {
    ...
    category: "video_processing",
    description:
      "Create Apple-compatible Live Photo outputs from product reference images or clone shot assets, with preview and export inside a dedicated plugin workspace.",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/live-photo-generator",
    usageHint:
      "Use a reference image to keep pose and scene while swapping only the product, or package clone storyboard assets as Live Photo exports.",
    configSchema: [
      {
        key: "retryLimit",
        label: "Maximum Image Attempts",
        type: "number",
        placeholder: "2",
        description: "Maximum automatic image generation retries. Minimum 0.",
      },
    ],
  },
  {
    ...
    category: "video_processing",
    description:
      "从复刻任务只读导入商品图和提示词，独立发起 TikTok Creative Studio 图生视频任务，不写回复刻主流程。",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/tiktok-creative-studio",
    usageHint:
      "适合把当前 clone 任务的商品图和提示词发送到 TikTok Creative Studio，生成和下载视频结果，同时与复刻主流程完全隔离。",
    configSchema: [
      {
        key: "imageRetryLimit",
        label: "Maximum Image Attempts",
        type: "number",
        placeholder: "1",
        description: "Maximum automatic reference image retries. Minimum 0.",
      },
    ],
  },
  {
    ...
    description:
      "上传商品实拍图后生成 TikTok 上架图片、标题、描述，并批量导出 Excel 模板。",
    version: "0.1.0",
    entryType: "tool",
    workspacePath: "/plugins/tiktok-listing-helper",
    usageHint:
      "适合独立处理 TikTok 商品图生成与店小秘导入模板导出，不影响 /clone 主流程。",
    configSchema: [
      {
        key: "defaultLanguage",
        label: "默认标题语言",
  ...
  },
  {
    ...
    usageHint:
      "默认走 Whisper 兼容 ASR + capcut-mate，ASS 导出保留为回退链路。",
    configSchema: [
      {
        key: "subtitleSource",
        label: "默认字幕来源",
      ...
      },
      {
        ...
        type: "boolean",
        description:
          "开启后，在 capcut-mate 失败时允许自动回退到 ASS 导出链路。",
      },
    ],
  },
  {
    ...
    category: "video_processing",
    description:
      "用于把复刻成片发布到 Geelark 云手机，并创建 TikTok 发布任务。",
    version: "0.1.0",
    entryType: "tool",
...
export function defaultPluginRecord(
  userId: string,
  pluginId: string,
): PluginRecord {
  return {
