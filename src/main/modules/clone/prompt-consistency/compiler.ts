...
function crossShotConsistencyText(shot: ShotSpec, modelPresentationShot: boolean) {
  return [
    ...
    modelPresentationShot
      ? 'No dialogue. No presenter delivery. Keep human demonstration context without turning the frame into a talking-head shot.'
      : 'No face. No dialogue.',
    buildReferenceImageLockText(),
    ...
    modelPresentationShot
      ? 'This shot must still read as a real human wearing or human demonstration shot.'
...
function modelIdentityPromptText(input: {
  ...
  return [
    ...
    input.modelIdentity?.description ? `Selected model identity description: ${input.modelIdentity.description}.` : '',
...
export function compilePromptConsistency(input: {
  ...
  const strictShotLayer = risk.strictConsistencyMode
    ? `Preserve the original shot logic only. ${normalized.scriptText}. Keep the product fully readable, visible, and structurally unchanged. Visual continuity must follow the reference images first.${modelPresentationShot ? ' This shot must remain a real human wearing or human demonstration shot, not an isolated product-only packshot or tabletop still life.' : ''}`
    : `Preserve the original shot logic. ${normalized.scriptText}. Visual direction: ${normalized.visualDescription}. Product focus: ${input.shot.productFocus || 'keep the product clearly visible and commercially relevant'}.${modelPresentationShot ? ' Keep visible human wearing or hand interaction context. Do not collapse this shot into an isolated static product image.' : ''}`
  ...
  const productDescriptionLayer = String(input.productDescription || '').trim()
    ? [
